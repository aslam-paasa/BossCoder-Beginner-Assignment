...
public class _08_InverseANumber {
    public static void main(String[] args) {
    
    /**    
     * Input : 426135
     * => Agar 6 digit ka sawal milega i.e. 426135 to uske andr jo digits
     *    honge wo v 1-6 digits tk honge bhlae hi utli pulti ho
     * => Koi v miss nhi hogi aur naa koi duplicate hogi
     * 
     * Input    : 6 digit h to 1 se 6 tk k saare digit aaenge
     ...
     * Number    : 4  2  6  1  3  5
     * Inverse   : 4  1  6  2  5  3
     * Note      : Basically it's like swapping index in array.
     * 
    ...
    */
        ...
        int inv = 0;
        /**
         * Jbtk 'n' 0 se bda h tbtk remainder nikaalna h
        */
        while(n > 0) {
            int rem = n % 10;
            int quo = n / 10;  // quotient (no chota krne k liye)
            n = quo;
            
            /**
             * r@p => p@r => p*pow(10, r-1) 
             * 1. remainder mila tha pth position pe (r@p) to humein
             *    'p' ko 'r' pe bhjna h (p@r).
             * 2. 'p'(position) ko 'r'(remainder) pe bhjne k liye p*pow(10, r-1)
             *    krna padega.
             * 3. Basically humein inverse ko 0 bna dena padega (int inv = 0)
             *    aur inverse ko badhaate jaenge.
             * 4. Math.pow decimal m jawab deta h to usko integer bnana pdega 
             *    (int)Math.pow()
            */ 
